...
//important variables
let mainWindow;
let excelFilePath = app.isPackaged
  ? path.join(
  ...
  : path.join(process.cwd(), "excel", "debtorsExceptions.xlsx");
let newQr;
let client;
...
ipcMain.handle("closeSession", async () => {
  client
    ...
    .catch((err) => {
      mainWindow.webContents.send("onSessionClose", err);
    });
});
...
ipcMain.handle("sendExcelBuffer", async (e, uint8arr, msgTemplate) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (!tel) {
        result.fallidos.push({
          ...
          number: tel,
          remainingDebt,
          reason: "Número vacío o inválido",
        });
        continue;
      }
    ...
    mainWindow.webContents.send("onMsgResult", {
      enviados: [],
      fallidos: [],
      error: String(err),
    });
    ...
    const getChromePath = () => {
      if (app.isPackaged) {
        try {
          ...
          console.error("Error resolving Chromium path:", e);
          return undefined;
        }
      } else {
        return undefined;
